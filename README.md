...
That led to a [formal treatment](https://marklubin.me/cdr.pdf) of what I think the structural constraints are. The core idea is that persistent memory under bounded resources faces three problems that aren't optional — compression (you must lose information, and the compression policy defines what the agent becomes), division (different information changes at different speeds, and a single namespace aliases them against each other), and renewal (every compression scheme degrades under distribution shift, and without active rebuilding the system silently rots).
