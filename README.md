...
I think the problem comes down to [three things](https://marklubin.me/cdr.pdf). Agents can't keep everything — context windows fill up, storage grows without bound — so they have to decide what to throw away, and that decision shapes what the agent becomes over time. They can't store today's conversation and long-term user preferences the same way at the same update rate — one overwrites the other. And whatever strategy they use to manage memory stops working as the world changes, so without periodic rebuilding, memory quietly goes stale. I call these compress, divide, and renew.
